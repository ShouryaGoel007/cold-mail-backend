...
def oauth2callback():
    ...
    # Save token as base64 string to send to frontend
...
def send():
    try:
        ...
        if not token_b64:
            return jsonify({"success": False, "error": "Not authenticated. Please connect Gmail first."}), 401

        resume_bytes = resume.read()
        resume_filename = resume.filename

        # Load user's personal token
